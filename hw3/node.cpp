...
{
    node *cursor;
    for (cursor = head_ptr; cursor != NULL && cursor->data() != target; cursor = cursor ->link());
    return cursor;
}
...
node *list_locate(node *head_ptr, std::size_t position)
{
    ...
    for (std::size_t i = 0; i < position && ans->link() != NULL; ++i)
        ans = ans->link();
...
void reverse(node* head_ptr)
{
    for (node* cursor = head_ptr; cursor != NULL; cursor = cursor->link())
        list_head_insert(cursor, cursor->data());
}

void remove_repetitions(node*head_ptr)
{
    for (node* cursor = head_ptr; cursor != NULL; cursor = cursor->link())
    {
...
node* add_lists(node *head_ptr, node *head_ptr2)
{
    if (head_ptr == NULL)
        return head_ptr2;
    if (head_ptr2 == NULL)
    ...
    for (node* cursor = head_ptr2; cursor != NULL; cursor = cursor->link())
        ans = new node(cursor->data(), cursor);

    for (node* cursor = head_ptr; cursor != NULL; cursor = cursor->link())
    {
        for (node* cursor2 = ans; cursor2 != NULL; cursor2 = cursor2->link())
        {
            if (cursor->data() == cursor2->data())
                break;
            if (cursor->data() < cursor2->data())
            {
                node* temp = new node(cursor->data(), cursor2);
                break;
            }
            if (cursor->data() > cursor2->data() && cursor->link() == NULL)
                list_insert(cursor2, cursor->data());
        }
    }
    return ans;
}
